...
import { BookOpen, HelpCircle, Volume2, RotateCw, Check, X, ArrowLeft, BrainCircuit, Code, Save, ChevronRight, Trophy, Meh, Smile, Loader2, Lock, Unlock, BarChart3, Clock, Flame, PieChart, AlertTriangle, Key, Facebook, Phone, Copy } from 'lucide-react';
...
type Mode = 'menu' | 'flashcard' | 'quiz' | 'listening' | 'code' | 'stats' | 'unlock';
...
// --- TTS HELPER (TFLAT STYLE) ---
const speakText = (text: string) => {
    // Hủy các lệnh đọc trước đó để tránh chồng chéo
    window.speechSynthesis.cancel();

    const utterance = new SpeechSynthesisUtterance(text);
    const voices = window.speechSynthesis.getVoices();
    
    // Ưu tiên các giọng đọc tự nhiên, chuẩn Mỹ (Google US English là giọng Tflat hay dùng trên Android)
    // Samantha là giọng chuẩn trên iOS/MacOS
    // Microsoft Zira là giọng chuẩn trên Windows
    ...
    utterance.rate = 0.85; // Tốc độ 0.85 giúp nghe rõ từng âm (giống từ điển)
    utterance.pitch = 1;   // Cao độ chuẩn
...
const EnglishHub: React.FC<EnglishHubProps> = ({ theme, user, onBack }) => {
  ...
  useEffect(() => {
    const defaultTopics: Topic[] = Array.from({ length: 17 }, (_, i) => ({
      id: `topic_${i + 1}`,
      title: `Unit ${i + 1}`, // Đổi thành Unit cho giống sách
  ...
  useEffect(() => {
      let interval: any;
      if (user && selectedTopic && mode !== 'menu' && mode !== 'stats' && mode !== 'unlock') {
  ...
  const handleSelectTopic = (topic: Topic) => {
    ...
    if (isLocked) {
        setMode('unlock');
        // Không set selectedTopic để giữ ở menu nhưng hiển thị modal unlock
        return;
    }
  ...
  const handleUnlock = async () => {
      ...
      try {
          ...
          if (inputCode.trim().toUpperCase() === serverCode) {
              ...
              setMode('menu'); // Quay lại menu để refresh trạng thái khóa
          } else {
  ...
  return (
    <div className="w-full mx-auto animate-in fade-in duration-500">
          ...
          <div className="flex flex-col items-center justify-center py-10 animate-in zoom-in-95 duration-300">
              <div className="bg-[#1e1e2e] p-8 rounded-3xl border border-pink-500/30 shadow-2xl max-w-md w-full relative overflow-hidden text-center">
                  ...
                  <input 
                      ...
                      placeholder="NHẬP MÃ (VD: VIP-2026)"
                      className="w-full bg-black/40 border border-gray-600 text-center text-xl font-mono text-white rounded-xl py-4 mb-6 focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none tracking-widest uppercase placeholder-gray-700"
                  />
                  ...
                  <div className="flex gap-3">
                      ...
                      >
                          {checkingCode ? <Loader2 className="animate-spin" size={20} /> : <Unlock size={20} />}
                          <span>Kích hoạt</span>
                      </button>
                  </div>
                  
                  {/* Footer Contact Info - UPDATED DESIGN */}
                  <div className="mt-8 w-full border-t border-white/5 pt-4 flex flex-col items-center">
                      <p className="text-[10px] text-gray-500 mb-3 font-medium flex items-center gap-1">
                          <AlertTriangle size={10} /> Liên hệ Admin nếu quên mã
                      </p>
                      <div className="flex gap-2 w-full justify-center">
                          <a 
                              ...
                              target="_blank" 
                              rel="noreferrer" 
                              className="flex-1 max-w-[140px] flex items-center justify-center gap-2 px-3 py-2.5 bg-[#1877F2]/10 hover:bg-[#1877F2]/20 text-[#1877F2] rounded-xl transition-all border border-[#1877F2]/20 text-xs font-bold group"
                          >
                              <Facebook size={16} className="group-hover:scale-110 transition-transform" /> Facebook
                          </a>
                          <button 
                              onClick={() => { navigator.clipboard.writeText('0795545909'); alert('Đã sao chép số Zalo: 0795545909'); }}
                              className="flex-1 max-w-[140px] flex items-center justify-center gap-2 px-3 py-2.5 bg-white/5 hover:bg-white/10 text-white/80 hover:text-white rounded-xl transition-all border border-white/10 text-xs font-bold group relative"
                              title="Sao chép số Zalo"
                          >
                              <Phone size={16} className="text-green-400 group-hover:rotate-12 transition-transform" /> 
                              <span>Zalo</span>
      ...
      {/* 4. LEARNING MODES (Only show if Selected Topic and NOT unlocking) */}
      {selectedTopic && mode !== 'unlock' && (
          <>
            {loading && (
